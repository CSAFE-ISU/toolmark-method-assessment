...
- The number of failed tests is highly dependent on the size of the comparison windows and is correlated to the ground truth, 
- Higher for known same-sourced lands than for known different sourced lands. 
- The \textbf{CS1} algorithm fails to conduct a test about 8 to 13 % of the time for known same-source lands, and 2 to 6% of the time for known different source lands.
- number of failed tests for CS1 is always higher than the corresponding theoretical number of failed tests.
...
- Even when the coarseness is high, CS2 will have lower number of failed tests than CS1, Making it more robust.
...
#### CS1
- Best works for $w_o$ of $\approx$ 130 to 160 and $w_v$ 50 when the smoothing is $c$ $\approx$ 0.3.
...
#### CS2
- For a window size of $w_o = 130$ we see a minimum in type II error rate across all type I rates considered. 
- Smaller validation sizes $w_v$ are typically associated with a smaller type II error.
...
- Might be able to get more power out of the test by adapting C2 in a bullet-to-bullet comparison.
