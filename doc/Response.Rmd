...
*Thank you for pointing this out. You are right, we have added more information about the data, wherever available. We are also citing xxx to emphasize the difficulty level and resulting differences in error rates of different brands of firearms and ammunition*
...
*You are right, a failure might occur for a match if the initial optimization fails. The new method is still susceptible to that. It would be interesting to further investigate thus problem in the furture. We have made a note of that in the conclusions*
...
*We followed the \cite{aoas} paper in extracting the profiles from scans. We have expanded on the discussion at the begining of section titled "Scans for land engraved areas" in paper*
...
*Thank you for poining this out, you are right in your understanding of the method. We have included a figure for this and expanded out discussion, which hopefully makes the it easier to understand the method.*
...
*Thank you for bringing this to our notice. We have included a table of error rates. The 93.5% to 94.1% are the True Negative rates which are being mentioned. It is computed as 1 - Observed FPR. The observed False positive rates can be seen in figure 10. The True positive rates would then be 1 - Observed FNR or about 64% to 72%.*
...
*Thank you for your comments, we appreciate your insight into this. This paper is a first measure in the adaption process of toolmark based methods to Bullets, and we therefore plan on making a comparison of this sort by including phase (as described by \cite{chu2010}) at a later stage by making such a comparison. In this paper we wanted to focus on the feasibility and possibility of making a successful adaption of the Chumbley method to bullet matching*

*We agree on the point that the toolmarks have large differences in width and the parameters of the chumbley algorithm are optimized for tools, which is why we first do a parameter study to identify the best set of parameters that would work for the LEAs. Before identifying problems in doing so and trying to address those problems*
...
*By failed test, what is being meant is that the method fails to conduct a statistical test although there is a resolution available in the ground truth as whether the lands match or dont match. This is because of incorrect identification of Maximum correlations windows required for comparison of the two striation marks. This means that there is no resolution as to the two striations being a match or a non-match, when there should be one *

*Thank you for pointing this out. For the terminology error rates, we do mean FPR = FP/ (TN + FP) and FNR = FN/ (TP + FN), which holds with the definition of error rates in literature.*

*Thank you for pointing out the confusing terminology as seen in the "Missed identifications" Page 17. missed same-source marking. *

*We are aware of the current standards of classification as having a section as inconclusive for examination. But for the data sets we are working on, each and every land has been identified and none of the data being used, cannot be classified as from the same-source and different source*
...
*By coarseness we mean the parameter used to identify smoothing span as used in the LOWESS filter.*
...
*We agree that having a Forensic examiner review the draft would bring value to the validation of the method. We would very politely like to bring to light that the work in this paper is incremental to the work done by \citet{hadler} and \citet{chumbley}. The objectivity of the methods were established in these works, and we only change the pairing of certain windows in the different-shift substep which does not change the format of the algorithm to compromise objectivity in the analysis*