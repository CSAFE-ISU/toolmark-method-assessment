...
\item Potential limitations in the application to striation marks on bullet lands: smaller in width and curved - need to adjust parameter settings
...
Figure \ref{fig:type1} compares nominal (fixed) type I error and actually observed type I errors for the parameter settings in table XXX. 
...
```{r, fig.width=8, fig.height=5, fig.cap="Number of failed tests."}
...
ns %>% filter(wv %in% c(30, 50)) %>% 
  ggplot(aes(x = wo, y = miss, shape=factor(wv), colour = factor(match))) + 
  geom_point() + 
  theme_bw() +
  ylab("Number of failed tests.")
```