...
The section \ref{appendix:appxfailed} of the Appendix discusses about the various scenarious of failed tests.
\begin{figure}[hbtp]
\centering
...
errorsCV.h44 <- rbind(errorrate(h44, 0.001), 
                   errorrate(h44, 0.005), 
                   errorrate(h44, 0.01), 
                   errorrate(h44, 0.05))

roc44.summary <- roc44.summary %>% left_join(
  errorsCV.h44 %>% dplyr::select(wv, wo, alpha, actual, beta) %>% 
    dplyr::filter(alpha == 0.05)
)
...
rocs44 <- rocs44 %>% dplyr::filter(wv==75)
