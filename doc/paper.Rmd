...
if (!file.exists("../data/invprofiles.rds")) {
  ...
  allinv$coarse <- allinv$coarseness
...
\autoref{fig:fails} shows the percentage of failed tests among the 85,491 land-to-land comparisons of the NIST data for different values of the validation window size $w_v$. For same-source lands up to 12.5 percent of the tests fail using CS1. The highest percentage of failed tests under CS2 is 0.9% for different source tests using a validation window size $w_v$ of 50 pixels. } 
...
fails %>% 
  ...
  ylim(c(0,13)) +
  facet_wrap(~method, labeller="label_method")
```
