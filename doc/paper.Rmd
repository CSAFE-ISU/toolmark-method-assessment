...
abstract: Same-source analyses are a major part of a Forensic Toolmark Examiner's job. Technological advances, such as profilometers and confocal microscopy allow to measure 3D surfaces in a high-resolution digitized form. Methods for matching marks for a variety of tools have been studied in the literature. Analysis of these digitized markings require the use of statistical methods which can quantify the scientific mechanism of comparing markings and serve as basis for an error rate calculation. In this paper, we are investigating the applicability of the Chumbley scoring method by \citet{hadler} to assess striation marks on bullet lands for same-source identification. The Chumbley score  provides us with another approach in the same-source assessment of bullet striation marks. 
---
...
Method & Data Source & False Positives & False Negatives\\
  \hline
Consecutive Matching Bullets Striae (CMS) & Bullets & & \\ \textbf{\citet{aoas}} &  & 6.25\% &  33.851\% \\ \hline
Consecutive Non-matching Striae (CNMS) & Bullets & & \\  \textbf{\citet{aoas}} & & 6.26\% & 35.4166\%  \\ \hline
Average Distance (D) & Bullets  &  & \\ \textbf{\citet{aoas}} & & 6.25\% & 45.833\% \\ \hline 
Cross-correlation Function (CCF) & Bullets & & \\ \textbf{\citet{aoas}}  & & 6.25\% & 17.7083\%  \\ \hline
Sum of Peaks (S) & Bullets  & & \\  \textbf{\citet{aoas}} & & 6.25\% &  18.22915\% \\
   \hline
\end{tabular}}
\caption{\label{tab:bullets-ER} Error Rates in same-source bullet analysis reported in the literature.}
\end{table}
...
\hh{What are we investigating in this paper, and what is the structure for the paper?}
\gk{In this paper we evaluate the adaptability of the chumbley score method to bullets, and in the process identify parameters that minimize the error rates. We then identify persistent scenarios in which the method proposed by \citet{hadler} fails to conduct a test. We go on to provide a solution to the failed tests problem, consequently increasing the power of the test and reducing error rates in the process. We also investigate the performance of the two algorithms using higher as well as lower resolution scans to understand if better scans and better equipment have a positive impact on the results.}
