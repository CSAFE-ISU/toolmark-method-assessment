...
errorratecoarse <- function(data, alpha) {
  summ <- data %>% filter(!is.na(p_value)) %>%
    mutate(signif = p_value < alpha) %>%
    group_by(wv, wo, coarse, match, signif) %>% tally()
  summ$error <- with(summ, match != signif)
  summ$alpha <- alpha
...
errors2 <- rbind(errorratecoarse(all, 0.001), 
                errorratecoarse(all, 0.005), 
                errorratecoarse(all, 0.01), 
                errorratecoarse(all, 0.05))
```
