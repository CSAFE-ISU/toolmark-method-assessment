...
ggerrors <- errorsCV %>%
  ggplot(aes(x = wo, y = beta, colour=factor(alpha), linetype=factor(alpha))) +
...
Results therefore do not transfer immediately to case work, where a forensic examiner would only deal with a few identifications. %Parameter settings for different studies should be investigated and reported to allow for a further refinement.
