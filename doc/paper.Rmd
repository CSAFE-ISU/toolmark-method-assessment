...
window_val <- 2*window_val*0.645

same.shift <- -(365-338)*1/.645 + sigs2.min - sigs1.min 
xs <- seq(450, 2000, length=10)
rects1 <- data.frame(ymin=-2.5, ymax= Inf, xmin = xs, xmax = xs+window_val)
rects2 <- data.frame(ymin=-Inf, ymax=-3.5, xmin = xs+same.shift, xmax = same.shift+xs+window_val)
rectsSS <- data.frame(rbind(rects1, rects2), type=rep(1:2, each=length(xs)))


p1 <- sigs_graphics[[1]] %>% filter(land_id %in% c(1, 276)) %>% 
  ggplot() + 
  geom_segment(aes(x = xmin, y=ymin, xend = xmin, yend=ymax), 
               colour="grey50", linetype=3, data = rectsSS) +
  geom_segment(aes(x = xmax, y=ymin, xend = xmax, yend=ymax), 
               colour="grey50", linetype=3, data = rectsSS) +
  geom_segment(aes(x = xmin, y=ymin, xend = xmin+same.shift, yend=ymin-1), 
               colour="grey50", linetype=3, data = rectsSS %>% filter(type==1)) +
  geom_segment(aes(x = xmax, y=ymin, xend = xmax+same.shift, yend=ymin-1), 
               colour="grey50", linetype=3, data = rectsSS %>% filter(type==1)) +
  geom_rect(aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, group=type), 
            data=rectsSS, fill= "grey90") +
  geom_line(aes(x = y, y=l30-7*I(land_id==276), group=land_id)) + 
  theme_bw() +
...
diff.shift <- -(365-338)*1/.645 + sigs2.min - sigs1.min + 50 
rects2 <- data.frame(ymin=-Inf, ymax=-3.5, xmin = xs+diff.shift, xmax = diff.shift+xs+window_val)
rectsDS <- data.frame(rbind(rects1, rects2), type=rep(1:2, each=length(xs)))

p2 <- sigs_graphics[[1]] %>% filter(land_id %in% c(1, 276)) %>% 
  ggplot() + 
  geom_segment(aes(x = xmin, y=ymin, xend = xmin, yend=ymax), 
               colour="grey50", linetype=3, data = rectsDS) +
  geom_segment(aes(x = xmax, y=ymin, xend = xmax, yend=ymax), 
               colour="grey50", linetype=3, data = rectsDS) +
  geom_segment(aes(x = xmin, y=ymin, xend = xmin+diff.shift, yend=ymin-1), 
               colour="grey50", linetype=3, data = rectsDS %>% filter(type==1)) +
  geom_segment(aes(x = xmax, y=ymin, xend = xmax+diff.shift, yend=ymin-1), 
               colour="grey50", linetype=3, data = rectsDS %>% filter(type==1)) +
  geom_rect(aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, group=type), 
            data=rectsDS, fill= "grey90") +
  geom_line(aes(x = y, y=l30-7*I(land_id==276), group=land_id)) + 
  theme_bw() +
  theme(axis.title.y = element_blank(), 
        axis.text.y = element_blank(),
