...
errorsCV <- rbind(errorrate(cvcs2, 0.001), 
...
#  facet_grid(.~wo, labeller="label_both") + 
 geom_line(aes(group=alpha),  size=.7) +
...
\autoref{fig:type22} gives an overview of ROC curves for methods CS1 and CS2 over a range of different optimization window sizes $w_o$. 

```{r type22, fig.height = 3.5, fig.width = 5, out.width='0.7\\textwidth', fig.cap="ROC curves of methods CS1 and CS2 for different sizes of optimization window $w_o$. Optimal ROC curves are reached at optimization windows of size 150 and higher. Points of equal error rates (EERs) can be found at the intersection of the dotted line and the ROC curves."}

cvcs2 %>% 
  ggplot(aes(d = as.numeric(!match), m = p_value, colour=factor(wo))) + 
  geom_roc(labels=FALSE, n.cuts=100, pointsize=0) +
  geom_abline(linetype=3, slope=-1, intercept = 1, colour="grey30", size=.5) +
  theme_bw() + 
  scale_colour_brewer(
...
XXX
still to cover: introduce method CS2;
failures are down; 
type 2 error rates are also down.
Fix has type 2 error rates on bullet lands that is still higher than the error rates achieved on the -much larger- toolmarks.
