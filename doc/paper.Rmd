...
Striation marks created on Bullets by the Barrel of a firearm are considered to be unique to the barrel of the firearm. This was first stated by the AFTE Criteria fo Identification Committee in 1992 (AFTE 1992 article). The maximum number of consecutively matching striae (CMS) as first seen by Biasotti 1959 and later mentioned in Chu et al. 2013 are the striated markings that line up exactly without any dissimilarity. This definition involves counting a single peak as a striae. A modified meaning of the terminology as explained by (Hare, Hofmann and Carriquiry) includes both peaks and valleys as means of identifying striae. Such identification methods more often than not include subject bia and are error prone and in case of CMS as seen in the work of Miller 1998 the number of CMS may turn to out to be high even when the bullet is not fires by the same firearm.

Thus, an important aspect of same source matching in firearms is identification of bullet microtopographies that are unique to be chosen for a match, Bullet profiles and signatures prove to be such features. The choice of a bullet signature to uniquely define a bullet depends on finding first, a stable region on a Bullet land which minimal noise and many pronounced striation marks. (Hare, Hofmann and Carriquiry)

The method employed by Hare, Hofmann and Carriquiry uses the cross correlation factor as a means to identify the stable region. The markings or striae in this region are supposed to have a high CCF with each other. Therefore a Bullet profile is chosen from this region by taking the cross section at a given height.
...
## How is the chumbley score computed?
...
This is computed from the joint rank of all correlations of both the same and different shift samples.
As given by Hadler et al.\citet{hadler}

Null Hypothesis: The toolmarks were not match i.e not made by the same tool.
...
$V = \dfrac{n_{s}n_{d}}{N\left(N-1\right)}\left[\Sigma^{ns}R_{s}\left(i\right)^{2}+\Sigma^{nd}R_{d}\left(j\right)^{2}\right] -\dfrac{n_{s} n_{d}\left(N+1\right)^{2}}{4\left(N-1\right)}$
 of the unnormalized U-statistic
...
sigs_graphics<- readRDS("./sigs_generate_window.rds")
