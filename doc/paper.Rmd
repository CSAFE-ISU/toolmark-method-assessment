...
if (!file.exists("../data/all-profiles.rds")) {
  ...
  for (file in files) {
    ...
    if (length(grep("x",names(tmp)) > 0)) tmp <- tmp %>% select(-x)
    allprofiles <- rbind(allprofiles, tmp)
  }
  allprofiles <- allprofiles %>% filter(land1_id < land2_id)
  allprofiles <- allprofiles %>% mutate(coarse = as.numeric(gsub("pt", ".", coarse)))
  saveRDS(allprofiles, file="../data/all-profiles.rds")
} else {
  allprofiles <- readRDS("../data/all-profiles.rds")
}

errors2 <- rbind(errorrate(allprofiles, 0.001), 
                errorrate(allprofiles, 0.005), 
                errorrate(allprofiles, 0.01), 
                errorrate(allprofiles, 0.05))
```
...
An important aspect of same source matching in firearms is identification of bullet microtopographies that are unique to be chosen for a match, Bullet profiles and signatures prove to be such features. The choice of a bullet signature to uniquely define a bullet depends on finding first, a stable region on a Bullet land with minimal noise and pronounced striation marks \citep{aoas}.
...
### Optimization step
The idea behind this step is to first identify the area of best agreement in the two toolmark data. Comparison window size is predefined by the user, which in case of screwdriver toolmarks was chosen by \citet{chumbley} and \citet{hadler} as around 10 percent of the length of the toolmarks, which was around 500. \hh{XXX it's not clear that the 500 is referring to the 10 percent.} This window is henceforth referred to as Window of optimization.

A maximum correlation statistic is used to identify the region of best agreement, with the maximum usually seen being near 1 for both cases which is what we intuitively expect for matches, and something which we do not intuitively expect for non matches \gk{done} \hh{XXX get rid of the parentheses - put the content into a sentence after your first statement.}

\hh{XXX the process you describe below is the cross-correlation of two series $x(t)$ and $y(t)$.}

First, there are a very large number of cross-correlations calculated for the two series of striae or marks $x(t_1)$ and $y(t_2)$, for eg if the window of optimization was defined as 200 and toolmark pixel length is around 1000 then we have 1200-(200-1) = 1001. Can be thought of ordered arrangements without replacement where n = 1001 and r = 1.
...
\hh{XXX Figure~\ref{win-comparison} is an orphan - cite it in the correct place.}
\hh{XXX let's include two or three more windows. We won't be able to use color - so we need to think of  a different way to distinguish between optimization and validation and link corresponding validation windows. Also include the code in the folder on the repo that generates sigs\_generate\_windows.rds }
...
#param_setting %>% tally() 
library(kableExtra)
knitr::kable(t(param_setting), digits=10, caption = "Overview of parameter settings used for optimization and validation windows for bullet land signatures.", format = "latex" ,booktabs=TRUE) %>% kableExtra::kable_styling(latex_options = c("scale_down"))
```
...
```{r, fig.cap="Type II errors for different levels of coarseness. XXX We need more values around .25"}
errors2 %>% ggplot(aes(x = coarse, y=beta, colour=factor(alpha))) +
...
```{r}
errors %>% ggplot(aes(x = wv, y=total, colour=factor(alpha), shape="signatures")) + geom_point() + facet_grid(~wo) +
  geom_point(data=errors2, aes(shape="profiles"))
```
## Discussion
