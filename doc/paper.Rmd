...
Digitized images, e.g. profilometers or confocal microscopy allow us to measure the surface \citep{vorburger2016} and can be used to conduct statistical analysis on the markings. Multiple toolmarks have been studied in the literature:  \citet{manytoolmarks1, chumbley} have studied screwdriver marks digitized using a profilometer,  \citet{manytoolmarks2} have investigated screwdriver, tongue and groove pliers 3D marks that are generated using a confocal microscope,  while, slip-joint pliers have been investigated by \citet{afte-chumbley} using digitized marks generated by a surface profilemeter. \citet{manytoolmarks2} define a relative distance metric and use it as similarity measure between two toolmarks. \citet{manytoolmarks1} used an algorithm which compares many small segments in the markings between two toolmarks. A maximum pearson correlation coefficient is then used as the statistical index for comparison. The Chumbley scoring method, first introduced by \citet{chumbley}, uses a similar but much more extenstive framework which includes a non-parametric statistical test. This was later improved by Hadler and Morris \citep{hadler}. In this paper, we are investigating the applicability of the Chumbley scoring method by \citet{hadler} to assessing striation marks on bullets for same-source identification.
