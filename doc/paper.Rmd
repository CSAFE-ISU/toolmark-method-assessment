...
\gk{ABSTRACT (150 words version)}
\newline
\gk{Same-source problem is a major challenge in forensic toolmark/ firearm examination. Technological advances in surface imaging allow us to measure 3D surfaces in previously unforeseen resolution and enable digitized images. Here, we investigate the applicability of the Chumbley scoring method by \citet{hadler}, developed for screwdriver markings, for same-source identification of bullet striation marks. We provide methods to identify parameters that minimize the error rates for matching of LEAs using Hamby datasets 44 and 252 measured by NIST and CSAFE. We then, provide a remedial algorithm to alleviate the problem of failed tests in the method, thereby increasing the power of the test and reducing error rates. Type II error rates of the proposed method showed an improvement of more than 20\% points (Type I error of 0.05) and are on average about 0.22. This puts the proposed method on similar footing as other single feature matching approaches in the literature.}\newline
\gk{Other potential \textbf{Keywords} : (minimum of 6)
profiles, signatures, same-source, different-source, comparison windows}
