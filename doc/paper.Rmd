...
pairs $(s_i^{(1)}, s_i^{(2)})$ for all integer values $i$ for which both $s_i^{(1)}$ and  $s_i^{(2)}$ are defined. Similarly, different-shift pairs are defined as $(s_i^{(1)}, s_{-i}^{(2)})$ for all $i$ where both $s_i^{(1)}$ and  $s_{-i}^{(2)}$ are defined.
