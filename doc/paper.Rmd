...
p2 <- profc %>% 
  ...
  theme_bw() +
  xlab(expression("Relative optimal location "~t^o)) +
  ylab("Number of profiles")
 
grid.arrange(p1, p2, ncol=1)
```

\hh{Figure \ref{fig:coarse} shows the type II error rates for profiles using an optimization window $w_o= 120$ and a validation window $w_v = 30$ for varying level of coarseness. The type II error for all nominal levels of  $\alpha$ are the lowest for a coarseness range of 0.20 to 0.30. For the remainder of the analysis we use a fixed value of coarseness of 0.25. }
