...
```{r }
library(kableExtra)
a27<- all %>% filter(wo==320, wv==50) %>% xtabs(data= ., ~signif+match)
...
<!-- \begin{minipage}{0.3\textwidth} -->
```{r}
a27<- all %>% filter(wo==120, wv==50) %>% xtabs(data= ., ~signif+match)
...
<!-- \end{minipage} -->
```{r}
a27<- all %>% filter(wo==80, wv==50) %>% xtabs(data= ., ~signif+match)
...
knitr::kable(t(param_setting)[,17:33], digits=0, format = "latex" ,booktabs=TRUE) %>% kableExtra::kable_styling(latex_options = c("scale_down","hold"))
...
errors_sig_c %>% ggplot(aes(x = coarse, y=total, colour=factor(alpha), shape="signatures")) + geom_point() + geom_line() + geom_point(data=errors2, aes(shape="profiles")) +geom_line(data=errors2, aes(shape="profiles"))
```
## Discussion
