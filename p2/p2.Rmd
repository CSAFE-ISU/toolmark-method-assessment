...
```{r, fig.cap="Type II errors for different levels of coarseness of profiles."}
...
```{r prof_missings, fig.width=8, fig.height=5, fig.cap="Number of failed tests by the window optimization size, wo, and ground truth. Test results from different sources have a much higher chance to fail, raising the question, whether failed tests should be treated as rejections of the null hypothesis of same source."}
...
#ns %>% filter(wv == c(30, 50)) %>%
  ...
  ylab("Percent of failed tests.") +
  xlab("Coarseness") +
  scale_colour_brewer("Match", palette="Set1") + geom_point(data=ns  %>% filter(wv %in% c(30,50)) , aes(shape="signature"), size = 2)# + scale_colour_brewer("Match", palette="Set2") #+geom_smooth(method="auto", size=.5, se=FALSE) 
  #scale_shape_discrete("Size of \nvalidation\nwindow, #wv")
...
- Profiles in comparison with Signatures seem to perform worse with total error either equal or more for all sizes of validation window.
- 
