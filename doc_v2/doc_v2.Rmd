...
The actual type I error and type II error for signatures were also compared for different validation window sizes. Figure \ref{fig:wo120} shows the actual rates for different nominal $\alpha$ levels. We can see that the actual $\alpha$ levels are well below the nominal rates for all validation windows, while the type II error rises with higher validation windows for the smaller nominal $\alpha$ levels while for the nominal $\alpha$ = 0.05 its almost constant.
```{r }
...
```{r missings, fig.width=5, fig.height=2, fig.cap="Number of failed tests by the window optimization size, wo, and ground truth.",fig.align = "center"}
...
```{r prof_missings, fig.width=10, fig.height=10, fig.cap="Bottom row shows Total error and Number of failed tests by the window validation size, wv, and ground truth, while the top row shows Total error and Number of failed tests with Coarseness for both profiles and signatures"}
